...
Level.prototype.isTeleport = function(pos,size)
{
    var xStart = Math.floor(pos.x);
    var xEnd = Math.ceil(pos.x + size.x);
    var yStart = Math.floor(pos.y);
...
Level.prototype.isJump = function(pos,size)
{
    var xStart = Math.floor(pos.x);
    var xEnd = Math.ceil(pos.x + size.x);
    var yStart = Math.floor(pos.y);
...
User.prototype.moveY = function(step, level, keys)
{
    ...
    //var teleport = level.isTeleport(newPos, this.size);
    //var jump = level.isJump(newPos, this.size);

    //If you're touching a wall, you can jump (because floor is considered wall)
    if(level.isWallAt(newPos, this.size))
    {
    ...
    //If you're touching lava, die and stuff
    ...
    //If you're touching teleporter, teleport
    if(level.isTeleport(newPos, this.size))
    {
        this.pos = new Vector(10,10);
    }

    //If you have a jump buff
...
Level.prototype.userTouched = function(type, actor)
{
    ...
    {
        this.actors.find(function(a){return a.type=="user"}).pos = this.actors.find(function(a){return a.type=="user"}).initPos;
    }
    else if (type == 'jump')
    {
        jumpSpeed=30;
        setTimeout(function(){jumpSpeed=20;},10000);
    }
};
...
//var gameLevel = new Level(GAME_LEVELS);
//console.log(gameLevel.width, "by", gameLevel.height);
...
function runLevel(level, Display, winlose)
{
